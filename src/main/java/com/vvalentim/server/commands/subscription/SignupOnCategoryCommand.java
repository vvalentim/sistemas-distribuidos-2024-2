...
public class SignupOnCategoryCommand extends Command {
    private final RequestSubscribeToCategory payload;

    public SignupOnCategoryCommand(RequestPayload payload) throws ClassCastException {
        this.payload = (RequestSubscribeToCategory) payload;
    }

    @Override
    public void execute() {
        if (payload.isInvalid()) {
            this.result = new ResponseFailedValidation(RequestType.SUBSCRIPTION_SUBSCRIBE_CATEGORY.jsonKey);
            return;
        }
        ...
        if (
            ...
            this.result = new ResponseUnauthorized(RequestType.SUBSCRIPTION_SUBSCRIBE_CATEGORY.jsonKey);
            return;
        }
        ...
        if (user == null) {
            this.result = new ResponseResourceNotFound(RequestType.SUBSCRIPTION_SUBSCRIBE_CATEGORY, ErrorType.USER_NOT_FOUND);
            return;
        }
        ...
        if (category == null) {
            this.result = new ResponseResourceNotFound(RequestType.SUBSCRIPTION_SUBSCRIBE_CATEGORY, ErrorType.NOTIFICATION_CATEGORY_NOT_FOUND);
            return;
        }

        db.subscribeOnCategory(this.payload.username, this.payload.categoryId);

        this.result = new ResponseSubscribedOnCategory("Cadastro em categoria realizado com sucesso.");
    }
}
